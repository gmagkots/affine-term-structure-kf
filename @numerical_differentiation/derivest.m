function [der,errest,finaldelta] = derivest(obj,fun,x0,varargin)
...
%
% Updates: NaN treatment added by Georgios Magkotsios (Jan 2012)
%

% check for NaNs in the input vector
nan_idx = find(isnan(x0),1);
if ~isempty(nan_idx)
    error('Input vector contains at least one NaN.');
end
...
% will we need fun(x0)?
% Calculate anyway, in case it is necessary to change differencing rule.
%if (rem(par.DerivativeOrder,2) == 0) || ~strncmpi(par.Style,'central',7)
  ...
  % check for NaNs in the function output at x0
  nan_idx = find(isnan(f_x0),1);
  if ~isempty(nan_idx)
      error('Input x0 resulted in f(x0) = NaN. Please change x0.');
  end
%else
%  f_x0 = [];
%end
...
for i = 1:n
  ...
  %
  % NaN treatment updates (Jan 2012):
  % change if-elseif structure and add if conditionals to enforce checking
  % all differencing rules. Start with the central rule, then proceed to
  % forward and backward rules if NaNs appear in the results.
  if par.Style(1) == 'c'
    ...
    % check for NaNs in the estimated function values, and change
    % differencing rule if at least one NaN appears
    nan_idx = find(isnan(f_del),1);
    if ~isempty(nan_idx)
        % change to forward differences
        par.Style = 'forward';
    end
  end
  ...
  if par.Style(1) == 'f'
    ...
    nan_idx = find(isnan(f_del),1);
    if ~isempty(nan_idx)
        % change to backward differences
        par.Style = 'backward';
    end
  end
  ...
  % In case f_del == NaN (scalar), convert f_del into a column vector of
  % NaN values and length ndel to avoid the length mismatch error.
  f_del = f_del(:);
  if isnan(f_del)
      f_del = NaN(ndel,1);
  end
