function [grad,err,finaldelta] = gradest(obj,fun,x0,varargin)
% gradest: estimate of the gradient vector of an analytical function of n variables
% usage: [grad,err,finaldelta] = gradest(fun,x0) or
%        [grad,err,finaldelta] = gradest(fun,x0,style)
%
...
%
%  style - same as the option used in derivest. If omitted
%          then it should have the default value of 'central'.
%          Added by Georgios Magkotsios (Jan, 2012).
%
% arguments: (output)
...
% check for Style argument (Georgios Magkotsios, Jan, 2012)
if (nargin == 3)
    style = 'central';
elseif (nargin == 4)
    if (strcmpi('central',varargin{1}) || ...
        strcmpi('forward',varargin{1}) || ...
        strcmpi('backward',varargin{1}) )
        style = varargin{1};
    else
        error('Argument "%s" is invalid for the type of differences.', ...
            varargin{1});
    end
else
...
finaldelta = grad;
for ind = 1:nx
  [grad(ind),err(ind),finaldelta(ind)] = derivest(obj, ...
    @(xi) fun(swapelement(x0,ind,xi)), ...
    x0(ind),'deriv',1,'vectorized','no', ...
    'methodorder',2,'style',style);
end
